# B2B Buyer Discovery — 外贸客户开发与开发信自动生成工具

> AI 驱动的外贸获客工具：自动搜索海外潜在买家 → 智能评分筛选高质量客户线索 → 提取邮箱和 WhatsApp → 自动生成多语种开发信。无需编程基础，只需复制几条命令，让 AI 帮你做客户开发。
...
> **适用场景**：B2B 外贸客户开发、cold email 营销、海外潜在买家挖掘、询盘线索收集。无论你是做机械设备、化工、建材、电子元器件还是消费品，只要修改关键词即可适配你的行业。

---

## 快速上手（新手只需 5 步）

不需要技术背景。如果你会复制粘贴命令行，5 分钟就能跑起来。
...
自动导出到桌面（路径可在 `.env` 中自定义），用 Excel 打开即可：
...
| 开发信预览 | 自动生成的西语/英语开发信草稿（意向分 ≥ 7 的客户） |

### 3. 开发信自动生成

意向分 ≥ 7 的高质量客户，工具会自动生成开发信草稿：
